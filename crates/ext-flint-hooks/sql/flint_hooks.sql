...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = flint, public, pg_catalog
AS $$
DECLARE
    wh          record;
    payload     jsonb;
...
BEGIN
    ...
    LOOP
        ...
        -- custom_headers is NOT NULL DEFAULT '{}', so this merge is always safe.
        ...
        IF wh.tier = 'standard' THEN
            -- Standard tier: fire-and-forget via pg_net (Supabase pg_net extension).
        ...
        ELSE
            -- Durable tier (implemented by p1-c003 BGW): queue in outbox.
            INSERT INTO flint.webhook_outbox
                (webhook_id, payload, headers, endpoint_url, status)
            VALUES
                (wh.id, payload, headers, wh.target_url, 'pending');
        END IF;
    END LOOP;
...
BEGIN
    FOR entry IN
        SELECT id, webhook_id, payload, headers, endpoint_url, retry_count
