...
use crate::compilers::filters::{cast_hints_for, parse_filter_tree, render_where_with_hints};
...
pub(super) async fn handle_list(
    ...
    let inner = match build_inner_query(&schema, &table, &params, &embed_schema, &hints) {
        Ok(inner) => inner,
...
pub(super) fn build_inner_query(
    ...
    // 2) Top-level WHERE, params continue after the projection params. `hints`
    // is the PARENT table's; embed-scoped child filters inside the projection
