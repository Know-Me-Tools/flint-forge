...
/// quantifier is applied to an operator that does not support it.
...
/// quotes are stripped and `\"`/`\\` unescaped. An empty list yields no elements
...
mod tests {
    ...
    fn render(col: &str, op: Operator, val: &str) -> (String, Vec<QueryParam>) {
        let (sql, params, _) = render_condition(col, op, val, false, None, 1).expect("render");
        (sql, params)
    }
    ...
    fn is_operator_rejects_bad_operand() {
        let err = render_condition("c", Operator::Is, "maybe", false, None, 1).unwrap_err();
        assert!(matches!(err, RenderError::InvalidIs(_)));
    }

    #[test]
    fn negation_wraps_condition() {
        let (sql, _, _) = render_condition("c", Operator::Eq, "1", true, None, 1).expect("render");
    ...
    fn quantifier_any_all_render() {
        let (sql, params, _) =
            render_condition("c", Operator::Eq, "(1,2)", false, Some(Quantifier::Any), 1)
                .expect("render");
        assert_eq!(sql, "c = ANY($1)");
        assert_eq!(params, vec![QueryParam::TextArray(vec!["1".into(), "2".into()])]);

        let (sql, _, _) =
            render_condition("c", Operator::Like, "(a%,b%)", false, Some(Quantifier::All), 1)
                .expect("render");
    ...
    fn quantifier_rejected_on_unsupported_operators() {
        for op in [Operator::In, Operator::Is, Operator::Cs, Operator::Ov] {
            let err = render_condition("c", op, "x", false, Some(Quantifier::Any), 1).unwrap_err();
            assert!(
    ...
    fn index_advances_by_bind_count() {
        let (_, _, next) = render_condition("c", Operator::Eq, "1", false, None, 5).expect("render");
        assert_eq!(next, 6, "one bind consumed");
        let (_, _, next) = render_condition("c", Operator::Is, "null", false, None, 5).expect("render");
