...
async fn test_base_component_count_at_least_50() {
    ...
    let row: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM flint_a2ui.components WHERE is_base = true")
            .fetch_one(&pool)
            .await
            .expect("query failed");
...
async fn test_all_seven_categories_present() {
    ...
    for cat in &categories {
        ...
        assert!(
            row.0,
            "category '{cat}' must have at least one base component"
        );
    }
}
...
async fn test_key_components_present() {
    ...
    for slug in &required {
        let row: (bool,) =
            sqlx::query_as("SELECT EXISTS (SELECT 1 FROM flint_a2ui.components WHERE slug = $1)")
                .bind(slug)
                .fetch_one(&pool)
                .await
                .expect("query failed");

        assert!(row.0, "required component '{slug}' must exist");
    }
}
