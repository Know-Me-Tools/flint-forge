...
- [ ] Make `agui_run` target configurable in `crates/ext-flint-hooks/sql/flint_hooks.sql:156` (default to localhost for dev, override for prod)
- [ ] Correct `crates/fdb-reflection/src/compilers/rest/mod.rs:62` doc-comment (CRUD is not `todo!()`)
- [ ] Correct `crates/fdb-reflection/src/compilers/rest/mod.rs:120-122` doc-comment (describe post-p16-c001 RLS behavior accurately)
- [ ] Correct `crates/fdb-gateway/src/main.rs:56-57` doc-comment re: keto field usage (verify current truth)
- [ ] Grep all flagged crates for similar stale-doc drift beyond the two called out; fix each
